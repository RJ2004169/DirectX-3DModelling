...
{
	m_persistance = 0.5f, m_octaves = 4;
	m_lacunarity = 0.5f, m_amplitude = 20.f, m_frequency = 0.07f;
	int index;
	float height = 0.0;
	bool result;
	...
	float textureCoordinatesStep = 20.0f / m_terrainWidth;  //tile 5 times across the terrain. 
	...
	for (int j = 0; j<m_terrainHeight; j++)
	{
		for (int i = 0; i<m_terrainWidth; i++)
		{
			...
			//m_heightMap[index]. = m_noise.generate_noise((float)i, (float)height, (float)j);
...
bool Terrain::GenerateHeightMap(ID3D11Device* device)
{
	...
	for (int j = 0; j<m_terrainHeight; j++)
	{
		for (int i = 0; i<m_terrainWidth; i++, amplitude = m_amplitude, frequency = m_frequency)
		{
			index = (m_terrainHeight * j) + i;

			m_heightMap[index].x = (float)i;
			noise2 = 0;
			for (int k = 0; k < m_octaves; k++)//, localPersistance-=10, localLacunarity-=0.01)
			{
				noise2 += (double)amplitude * m_noise.generate_noise((float)i * frequency, (float)j * frequency, 1.f);
				frequency *= m_lacunarity;
				amplitude *= m_persistance;
				//noise2 += noise1;
			}
			m_heightMap[index].y = noise2;//((float)20 * m_noise.generate_noise((float)i * 0.07, (float)j * 0.07, 1.f)) + ((float)40 * m_noise.generate_noise((float)i * 0.07, (float)j * 0.07, 1.f));
			m_heightMap[index].z = (float)j;
		}
	}
...
DirectX::SimpleMath::Vector3 Terrain::TerrainHeightAt(DirectX::SimpleMath::Vector3 cameraPosition)
{
	/*int index = 0;
	for (int j = 0; j < m_terrainHeight; j++)
	{
		for (int i = 0; i < m_terrainWidth; i++)
		{
			...
			if (m_heightMap[index].x == xPosition)
			{
	...
	int index = 0;
	index = (m_terrainHeight * cameraPosition.x) + cameraPosition.y;
